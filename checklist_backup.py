...
async def pdf_survey(request: Request, kantor_code: str, session: Optional[str] = Cookie(None)):
    user = get_user_from_cookie(session)
    if not user:
        return HTMLResponse(status_code=302, headers={"Location": "/login"})
    try:
        ...
        cur.execute(
            """SELECT kantor_code, pic, tgl_cek, status_data, yes_count, total_items, 
                      nomor_survei, survey_seq, workflow_status, created_at, submitted_at
               FROM origo.kantor_checklist_data 
               WHERE kantor_code = %s 
               ORDER BY survey_seq DESC LIMIT 1""",
            (kantor_code,)
        ...
        kode, pic, tgl, sd, yes, tot, nomor, seq, wf, ca, sa = row
        items = sd if sd else []
...
<style>
  ...
  .meta {{ margin:10px 0; font-size:10pt; }}
</style></head><body>
<h1>Hasil Survei Ceklist Kantor</h1>
<div class="meta">
  <strong>Kantor:</strong> {kode} &nbsp;|&nbsp; <strong>PIC:</strong> {pic or '-'} &nbsp;|&nbsp; <strong>Tanggal:</strong> {tgl_str}<br>
  <strong>Nomor Survei:</strong> {nomor or '-'} &nbsp;|&nbsp; <strong>Status:</strong> {wf_label} &nbsp;|&nbsp; <strong>Skor:</strong> {skor}%
</div>
<table><thead><tr><th>#</th><th>Kategori</th><th>Item</th><th style="width:50px">Nilai</th><th>Catatan</th></tr></thead><tbody>"""]

        vlbl = {0:"✅", 1:"⚠️", 2:"❌", 3:"❌", 4:"❌"}
        for i, it in enumerate(items):
            if i >= len(db_items):
                html_parts.append(f"<tr><td>{i+1}</td><td colspan='4'>(item ekstra)</td></tr>")
                continue
            di = db_items[i]
            ...
            html_parts.append(
                f"<tr><td>{i+1}</td><td>{di['cat']}</td><td>{di['label']}</td>"
                f"<td style='text-align:center'>{label}</td><td>{note or '-'}</td></tr>"
            )

        html_parts.append("""</tbody></table>
<p style="margin-top:10px;font-size:8pt;color:#666;">
Dicetak: """ + dtdt.now().strftime("%d-%m-%Y %H:%M") + """ WIB</p></body></html>""")
