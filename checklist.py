...
async def form(request: Request, kantor_code: str, session: Optional[str] = Cookie(None)):
    ...
    if workflow_status in ("draft", "submitted", "final"):
        try:
            ...
            cur3 = conn3.cursor()
            cur3.execute(
                "SELECT status_data FROM origo.kantor_checklist_data WHERE kantor_code=%s AND workflow_status IN ('draft', 'submitted') ORDER BY survey_seq DESC LIMIT 1",
                (kantor_code,)
            )
            srow3 = cur3.fetchone()
...
async def api_load_session(
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT * FROM origo.kantor_checklist_data WHERE kantor_code=%s ORDER BY survey_seq DESC LIMIT 1",
            (kantor_code,),
        )
...
async def survey_dashboard(request: Request, session: Optional[str] = Cookie(None), sort: str = Query(""), dir_order: str = Query("asc")):
    ...
    try:
        ...
        # Ambil timestamp laporan terakhir
        conn_ts = get_db(); cur_ts = conn_ts.cursor()
        cur_ts.execute("""
            SELECT MAX(GREATEST(
                COALESCE(updated_at, '1970-01-01'::timestamp),
                COALESCE(submitted_at, '1970-01-01'::timestamp)
            ))
            FROM origo.kantor_checklist_data
            WHERE yes_count IS NOT NULL AND total_items > 0
        """)
        last_ts = cur_ts.fetchone()[0]
        cur_ts.close(); conn_ts.close()
        if last_ts and last_ts.year > 1970:
            last_report_update = last_ts.strftime('%d-%m-%Y %H:%M WIB')
        else:
            last_report_update = 'Belum ada data'
...
async def item_stats_api(request: Request, session: Optional[str] = Cookie(None),
    ...
    try:
        ...
        # Timestamp laporan: MAX submitted_at/updated_at dari semua session yg difilter
        _tmp_conn = get_db(); _tmp_cur = _tmp_conn.cursor()
        _tmp_cur.execute("""
            SELECT MAX(GREATEST(COALESCE(updated_at, '1970-01-01'), COALESCE(submitted_at, '1970-01-01')))
            FROM origo.kantor_checklist_data
            WHERE workflow_status IN ('submitted','reviewed','approved')
        """)
        _lr = _tmp_cur.fetchone()[0]
        if _lr:
            _lr = _lr.astimezone(__import__('zoneinfo').ZoneInfo("Asia/Jakarta"))
            last_ts_str = _lr.strftime('%d-%m-%Y %H:%M WIB')
        else:
            last_ts_str = 'Belum ada data'
        _tmp_cur.close(); _tmp_conn.close()
...
async def item_evidence_api(request: Request):
    ...
    wilayah_code = request.query_params.get("wilayah_code", "")
    ...
    try:
        ...
        # Filter area/wilayah: ambil mapping tree dulu
        _branch_area = {}
        if area_code or wilayah_code:
            _cur2 = conn.cursor()
            _cur2.execute("""
                WITH RECURSIVE tree AS (
                    SELECT n.id, n.parent_id, n.office_code, n.branch_kind,
                           1 AS depth, CAST(n.node_code AS text) AS path_code
                    FROM origo.network_tree_node n
                    WHERE n.version_id = (SELECT id FROM origo.network_tree_version WHERE is_published = TRUE LIMIT 1)
                ...
                SELECT office_code, path_code FROM tree WHERE office_code IS NOT NULL AND branch_kind NOT IN ('area','wilayah')
            """)
            for _r in _cur2.fetchall():
                pc = (_r[1] or "").split(">")
                _branch_area[_r[0]] = {
        ...
        for r in cur.fetchall():
            ...
            # Filter area/wilayah
            if area_code or wilayah_code:
                ba = _branch_area.get(kantor_code, {})
                if area_code and ba.get("area_code") != area_code:
                    continue
                if wilayah_code:
                    ba_wil = ba.get("wilayah_code", "")
                    comp_code = ba.get("area_code", "") + ">" + ba_wil
                    if ba_wil != wilayah_code and comp_code != wilayah_code:
                        continue
